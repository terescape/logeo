...
//import { SVGLoader } from 'https://unpkg.com/three@0.119.0/build/three.module.js';
...
const addGeoObject = function ( group, svgObject ) {
    ...
    for ( let i = 0; i < paths.length; i ++ ) {
        ...
        const path = paths[i];
        const color = new THREE.Color("rgb(200, 200, 255)");        //THREE.Color( colors[ i ] );     //
        const material = new THREE.MeshLambertMaterial( {
            color: color,
            //emissive: color
        } );
        const depth = depths[ i ];
        const simpleShapes = path.toShapes( false );
...
function init() {
    ...
    renderer.setSize( LOGO_WIDTH * window.innerWidth/window.innerHeight, LOGO_WIDTH);
